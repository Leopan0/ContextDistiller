...
function installCompactRouter(ctx, get) {
  return ctx.on(
    ...
    (options, next) => {
      ...
      const rerouted = deepFreeze2({ ...options, provider: route.provider, model: route.model });
...
function apply(ctx, config) {
    ...
    handler: async (req, res) => {
      ...
      if (req.method === "POST") {
        try {
          ...
          const r = resolved();
          ctx.logger.info(
            `context-distiller config updated (router: ${r.compact.enabled ? "on" : "off"}, provider: ${r.compact.provider}, model: ${r.compact.model})`
          );
          res.writeHead(200, { "content-type": "application/json" });
  ...
  ctx.logger.info(
    `context-distiller loaded (router: ${resolved().compact.enabled ? "on" : "off"}, engine: ${resolved().engine.enabled ? "on" : "off"})`
  );
}
export {
  Config,
